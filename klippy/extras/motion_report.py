...
class DumpTrapQ:
    ...
    def log_trapq(self, data):
        ...
        for i, m in enumerate(data):
            out.append("move %d: pt=%.6f mt=%.6f sv=%.6f a=%.6f"
                       " sp=(%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f) ar=(%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f)"
                       % (i, m.print_time, m.move_t, m.start_v, m.accel,
                          m.start_x, m.start_y, m.start_z, m.start_w,
                          m.start_a, m.start_b, m.start_c, m.start_d,
                          m.x_r, m.y_r, m.z_r, m.w_r, m.a_r, m.b_r, m.c_r, m.d_r))
        logging.info('\n'.join(out))
    def get_trapq_position(self, print_time):
        ...
        pos = (move.start_x + move.x_r * dist, move.start_y + move.y_r * dist,
               move.start_z + move.z_r * dist, move.start_w + move.w_r * dist,
               move.start_a + move.a_r * dist, move.start_b + move.b_r * dist,
               move.start_c + move.c_r * dist, move.start_d + move.d_r * dist)
...
class PrinterMotionReport:
    def __init__(self, config):
        ...
        self.last_status = {
            'live_position': gcode.Coord(0., 0., 0., 0., 0., 0., 0., 0., 0.),
