...
class PrinterExtruder:
    ...
    def check_move(self, move):
        ...
        if (not move.axes_d[0] and not move.axes_d[1]) or axis_r < 0.:
            ...
            if abs(move.axes_d[8]) > self.max_e_dist:
                raise self.printer.command_error(
                    "Extrude only move too long (%.3fmm vs %.3fmm)\n"
                    "See the 'max_extrude_only_distance' config"
                    " option for details" % (move.axes_d[8], self.max_e_dist))
            inv_extrude_r = 1. / abs(axis_r)
            move.limit_speed(self.max_e_velocity * inv_extrude_r,
                             self.max_e_accel * inv_extrude_r)
        elif axis_r > self.max_extrude_ratio:
            if move.axes_d[8] <= self.nozzle_diameter * self.max_extrude_ratio:
    ...
    def calc_junction(self, prev_move, move):
        diff_r = move.axes_r[8] - prev_move.axes_r[8]
        if diff_r:
    ...
    def move(self, print_time, move):
        ...
        self.trapq_append(self.trapq, print_time,
                          ...
                          move.start_pos[8], 0., 0., 0., 0., 0., 0., 0.,
                          1., can_pressure_advance, 0., 0., 0., 0., 0., 0.,
